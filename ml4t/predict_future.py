...
def predict_spy_future(horizon=5):
	...
	for sym in spy_list:
		...
		train_rows = int(math.floor(0.8* data.shape[0]))
		...
		future_pred = learner.query(todays_values)
	...
	results	= results.sort_values(by=["Test Error (RMSE)"])
